...
if(await data.fetch(`slm.${message.author.id}.${message.guild.id}.aşk`)) return message.channel.send(new Discord.MessageEmbed().setColor('RED').setDescription(`**20 saniyenin altında hızlı kullanamazsın!**`));
...
data.set(`slm.${message.author.id}.${message.guild.id}.aşk`, 'NEA');
setTimeout(() => {
